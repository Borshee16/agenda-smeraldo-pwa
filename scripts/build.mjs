...
const sw=await readFile('sw.js','utf8');hash.update(sw);await writeFile('dist/sw.js',sw.replace(/const VERSION='[^']*'/, "const VERSION='"+hash.digest('hex').slice(0,16)+"'"));await writeFile('dist/.nojekyll','');console.log('PWA pronta in dist/.');
